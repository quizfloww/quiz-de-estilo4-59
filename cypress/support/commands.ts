...
Cypress.Commands.add('navigateToFunnelEditor', (funnelId?: string) => {
  ...
  cy.get('[data-testid="funnel-editor-container"]', { timeout: 15000 }).should('be.visible');
});
...
Cypress.Commands.add('dragBlockToPosition', (blockId: string, x: number, y: number) => {
...
Cypress.Commands.add('resizePanel', (panelId: string, direction: 'left' | 'right' | 'top' | 'bottom', pixels: number) => {
...
Cypress.Commands.add('openBlockSettings', (blockId: string) => {
  ...
  cy.get('[data-testid="block-settings-menu"] button:contains("Configurações")').click();
});
...
Cypress.Commands.add('publishFunnel', () => {
  ...
  cy.get('[data-testid="publish-success-message"]', { timeout: 10000 }).should('be.visible');
});
...
declare global {
  namespace Cypress {
    interface Chainable {
      ...
      dragBlockToPosition(blockId: string, x: number, y: number): Chainable<void>;
      resizePanel(panelId: string, direction: 'left' | 'right' | 'top' | 'bottom', pixels: number): Chainable<void>;
