...
export const CtaOfferBlock: React.FC<CtaOfferBlockProps> = ({ content, isPreview }) => {
  const text = content.ctaText || 'GARANTIR MEU GUIA AGORA';
  const url = content.ctaUrl || '#';
  const variant = content.ctaVariant || 'green';
  const urgencyText = content.urgencyText || 'O preço pode aumentar a qualquer momento';
  ...
  const variantClasses = {
    ...
    brand: 'bg-gradient-to-r from-[#B89B7A] to-[#9A7B5A] hover:from-[#A68B6A] hover:to-[#8A6B4A] shadow-[#B89B7A]/30',
    gradient: 'bg-gradient-to-r from-purple-500 via-pink-500 to-red-500 hover:from-purple-600 hover:via-pink-600 hover:to-red-600 shadow-pink-500/30',
  };
